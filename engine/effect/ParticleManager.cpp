...
void ParticleManager::Fire(Particle* particle_,XMFLOAT3 pos, const float& setpos, const float& setvel, const float& setacc, const int& setnum, const XMFLOAT2& setscale)
{
	for (int i = 0; i < setnum; i++)
	{
		//X,Y,Z全て{-20.0f,20.0f}でランダムに分布
		const float md_pos = setpos;
		XMFLOAT3 ppos = {pos.x,pos.y,pos.z};
		ppos.x += (float)rand() / RAND_MAX * md_pos - md_pos / 2.0f;
		ppos.y += (float)rand() / RAND_MAX * md_pos - md_pos / 2.0f;
		...
		//追加
		particle_->Add(60, ppos, vel, acc, setscale.x, setscale.y);
	}
}